        ...
        function Recurse-Smo {
                    ...
                    if (@($nextProperties).Count -gt 1) {
                        if ($propertyValue -is [System.Collections.ICollection] -and $propertyValue.Count -gt 0) {
                            foreach ($item in $propertyValue.GetEnumerator()) {
                                ...
                                $Output = Recurse-Smo $item $OutputObject $Depth $path $propertyName $primaryKeyColumns
                            }
                        } elseif ($isArray -and $tableName -eq "Configuration") {
                            ...
                            foreach ($item in @($propertyValue)) {
                                ...
                                $Output = Recurse-Smo $item $OutputObject $Depth $path $propertyName $parentPrimaryKeyColumns
                            }    
                        } elseif ($isArray) {
                            foreach ($item in @($propertyValue)) {
                                Write-Verbose "$($tab)Recursing through array node"
            ...
            if (!$table.PrimaryKey) {
                try { 
                    ...
                    Set-PSBreakpoint -Variable StackTrace
                } 
